...
final class BattleFrame extends JFrame {
    ...
    void askPlayerPickTeam() {
        ...
        // Panel with checkboxes (2 columns so sprites show compactly)
        JPanel boxPanel = new JPanel();
        boxPanel.setLayout(new GridLayout(0, 2, 8, 8)); // 2 columns grid
        JCheckBox[] boxes = new JCheckBox[pokedex.size()];

        for (int i = 0; i < pokedex.size(); i++) {
            Pokemon p = pokedex.get(i);

            // Build a simple HTML label so text is multi-line and formatted
            String labelHtml = String.format("<html><b>%s</b> [%s]<br/>HP: %d &nbsp;&nbsp; Atk: %d</html>",
                    p.name, p.type, p.maxHp, p.attack);

            ImageIcon small = scaledIcon(p.sprite, 80, 80);
            JCheckBox box = new JCheckBox(labelHtml, small);
            boxes[i] = box;
            ...
            // Blue colour when selected
            Color normal = Color.WHITE;
            Color blue = new Color(180, 205, 255); 
            Color blueBorder = new Color(40, 90, 200);
            box.setBackground(normal);
            box.setBorder(BorderFactory.createLineBorder(Color.GRAY, 1));
            box.addItemListener(e -> {
                if (box.isSelected()) {
                    box.setBackground(blue);
                    box.setBorder(BorderFactory.createLineBorder(blueBorder, 2));
                } else {
                    box.setBackground(normal);
                    box.setBorder(BorderFactory.createLineBorder(Color.GRAY, 1));
                }
    ...
    void initPokedex() {
        pokedex.add(new Pokemon("Charmander", PokeType.FIRE, 72, 55, movesOf(PokeType.FIRE)));
        pokedex.add(new Pokemon("Vulpix", PokeType.FIRE, 72, 53, movesOf(PokeType.FIRE)));
        pokedex.add(new Pokemon("Growlithe", PokeType.FIRE, 80, 50, movesOf(PokeType.FIRE)));
        pokedex.add(new Pokemon("Fennekin", PokeType.FIRE, 70, 60, movesOf(PokeType.FIRE)));
        pokedex.add(new Pokemon("Torchic", PokeType.FIRE, 70, 60, movesOf(PokeType.FIRE)));

        pokedex.add(new Pokemon("Squirtle", PokeType.WATER, 75, 55, movesOf(PokeType.WATER)));
        pokedex.add(new Pokemon("Froakie", PokeType.WATER, 70, 60, movesOf(PokeType.WATER)));
        pokedex.add(new Pokemon("Totodile", PokeType.WATER, 80, 50, movesOf(PokeType.WATER)));
        pokedex.add(new Pokemon("Piplup", PokeType.WATER, 70, 60, movesOf(PokeType.WATER)));
        pokedex.add(new Pokemon("Oshawott", PokeType.WATER, 75, 55, movesOf(PokeType.WATER)));

        pokedex.add(new Pokemon("Bulbasaur", PokeType.GRASS, 80, 50, movesOf(PokeType.GRASS)));
        pokedex.add(new Pokemon("Oddish", PokeType.GRASS, 70, 60, movesOf(PokeType.GRASS)));
        pokedex.add(new Pokemon("Snivy", PokeType.GRASS, 70, 60, movesOf(PokeType.GRASS)));
        pokedex.add(new Pokemon("Treecko", PokeType.GRASS, 72, 53, movesOf(PokeType.GRASS)));
        pokedex.add(new Pokemon("Chikorita", PokeType.GRASS, 75, 55, movesOf(PokeType.GRASS)));

        pokedex.add(new Pokemon("Meowth", PokeType.NORMAL, 72, 53, neutralMoves()));
        pokedex.add(new Pokemon("Raticate", PokeType.NORMAL, 80, 50, neutralMoves()));
        pokedex.add(new Pokemon("Eevee", PokeType.NORMAL, 70, 60, neutralMoves()));
        pokedex.add(new Pokemon("Pikachu", PokeType.NORMAL, 75, 55, neutralMoves()));
        pokedex.add(new Pokemon("Gible", PokeType.NORMAL, 70, 60, neutralMoves()));
    }
    ...
    void loadSprites() {
        ...
        for (Pokemon p : pokedex) {
            String fname = "resources/sprites/" + p.name.toLowerCase().replace(' ', '_') + ".png";
